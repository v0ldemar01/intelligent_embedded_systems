...
def calculate_time_correlation(signal_harmonics, start_frequency, number_discrete_calls, type_correlation):
    time = []
    for n in range(number_discrete_calls):
        signal_first = signal_generator.generate_signal(signal_harmonics, start_frequency, n + 1)
        signal_second = signal_generator.generate_signal(signal_harmonics, start_frequency, n + 1)
        start_time = t.time()
